...
export function PendingApplicationsWidget() {
  const { data: appResponse, isLoading } = useQuery({
    queryKey: ['recent-applications-widget'],
    queryFn: () => applicationsApi.findAll({ limit: 5, sortBy: 'createdAt', sortOrder: 'desc' }),
  });
    ...
    <Card className="border-neutral-200">
      <CardHeader className="flex flex-row items-center justify-between border-b p-6 bg-neutral-50/20">
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">Latest applications visible to your HR role.</p>
        </div>
        <Link
          href="/applications"
      ...
      <CardContent className="p-6">
        {isLoading ? (
          ...
          <div className="text-center py-8">
            <p className="text-sm text-muted-foreground">No applications at the moment.</p>
          </div>
        ) : (
