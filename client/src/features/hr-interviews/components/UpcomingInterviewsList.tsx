    ...
    <Card className="border-neutral-200 shadow-none h-full flex flex-col">
      <CardHeader className="bg-neutral-50/30 border-b p-4 shrink-0">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="secondary" className="bg-white border text-neutral-600">
              ...
              <div key={interview.id} className="p-4 hover:bg-neutral-50/50 transition-colors">
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-neutral-900">{interview.candidateName}</h4>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[10px] uppercase font-bold tracking-wider text-neutral-500 bg-neutral-100 px-1.5 py-0.5 rounded">
                          {interview.applicationCode}
                        </span>
                        <span className="text-xs text-neutral-600">{interview.department?.name || 'General'}</span>
                      </div>
                      
                      <div className="flex items-center gap-3 mt-2.5 text-xs font-medium">
                        <div className="flex items-center gap-1.5 text-neutral-700 bg-white border px-2 py-1 rounded-md shadow-sm">
                          <Calendar className="h-3.5 w-3.5 text-neutral-400" />
                          {new Date(interview.slot_date).toLocaleDateString()}
                        </div>
                        <div className="flex items-center gap-1.5 text-neutral-700 bg-white border px-2 py-1 rounded-md shadow-sm">
                          <Video className="h-3.5 w-3.5 text-blue-500" />
                          {(() => {
                            try {
                              return new Date(interview.slot_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                            } catch {
                              return interview.slot_time;
                            }
                          })()}
                        </div>
                      </div>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-neutral-400 cursor-pointer">
